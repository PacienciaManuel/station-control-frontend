...
const columns: GridColDef[] = [
    ...
    { field: 'nome', headerName: 'Nome', sortable: false, type: 'string', width: 240, renderCell: ({value}: GridRenderCellParams<any, Suspeito>) => (
        <Stack spacing={1} direction="row" sx={{alignItems: "center"}}>
            <Avatar src={resourceResolver.profilePhoto(value?.foto)} alt={value!.nome} sx={{color: "#fff", fontSize: rem(16), width: 24, height: 24, backgroundColor: toolsManager.generateColor(value!.nome)}}>{value!.nome.charAt(0).toUpperCase()} </Avatar>
    ...
    { field: 'dataAtualzacao', headerName: 'Última Atualização',type: 'dateTime', width: 140},
    { field: 'totalOcorrencias', headerName: 'Ocorrências',type: 'actions', width: 100, description: 'Este é o total de ocorrências registradas por suspeito.', renderCell: ({value}: GridRenderCellParams<any, Suspeito>) => (
        <SuspectOccurrencesHover suspeito={value!} anchorOrigin={{vertical: 'top', horizontal: 'center',}} transformOrigin={{vertical: 'bottom', horizontal: 'center',}}>
        ...
        </SuspectOccurrencesHover>
    )},
    { field: 'pais', headerName: 'Pais',type: 'string', width: 100, description: 'Este é a nacionalidade dos suspeitos', renderCell: ({value}: GridRenderCellParams<any, string>) => (
        <Tooltip bg={'black'} label={value} position='top' transitionProps={{transition: 'slide-down'}} withArrow>
            <Typography variant="body2">{value}</Typography>
        </Tooltip>
    )},
    { field: 'actions', headerName: 'Acções', sortable: false, type: 'actions', width: 120, description: 'Veja os detalhes das ocorrências.', renderCell: ({value}: GridRenderCellParams<any, Actions>) => (
        <Stack spacing={1} direction="row">
            <Tooltip bg={'black'} label="Ver todos Detalhes" position='top' transitionProps={{transition: 'slide-down'}} withArrow>
                <IconButton LinkComponent={NextLink} href={`/applicants/${value?.suspeito.id}`}>
            ...
            <Tooltip bg={'black'} label="Excluir" position='top' transitionProps={{transition: 'slide-down'}} withArrow>
                <IconButton color="error" onClick={() => value.handleDelete!(value!.suspeito)}>
...
export default function Suspects({suspeitos, title, handleDelete, total=0}:SuspectsProps) {
    return (
        <Stack elevation={0} component={Paper} sx={{p: 2, }}>
            ...
            <DataGrid
                rows={suspeitos?.map(suspeito => ({
                    id: suspeito.id,
                    nome: suspeito,
                    genero: suspeito.genero,
                    dataNascimento: new Date(suspeito.dataNascimento),
                    morada: suspeito.morada,
                    bilheteIdentidade: suspeito.bilheteIdentidade,
                    dataCriacao: new Date(suspeito.dataCriacao),
                    dataAtualzacao: new Date(suspeito.dataCriacao),
                    totalOcorrencias: suspeito,
                    pais: suspeito.pais.nome,
                    actions: {suspeito, handleDelete} as Actions,
                })) || []}
                columns={columns}
                initialState={{
                    pagination: {
